...
export class QuestAPI 
{
    ...
    {
        const questConfig = this.Instance.configServer.getConfig<IQuestConfig>(ConfigTypes.QUEST)
        const questSideFile = fs.readFileSync(this.Instance.dbPath.concat("/Quests/QuestSideData.json"), "utf-8");
        const questSideJson = JSON.parse(questSideFile);

        this.Instance.logger.log(`[${this.Instance.modName}] QuestAPI:  ${questSideJson["UsecOnly"]}`, LogTextColor.BLUE);
